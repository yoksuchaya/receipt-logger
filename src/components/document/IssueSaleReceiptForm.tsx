...
type IssueSaleReceiptFormProps = {
...
const IssueSaleReceiptForm: React.FC<IssueSaleReceiptFormProps> = ({ initialValues, mode = 'create', onSubmit, onCancel }) => {
    ...
    async function updateReceiptNo(form: FormState, isDateChange: boolean) {
        ...
        let bankCode = '';
        if (companyProfile?.paymentBankOptions) {
            const found = companyProfile.paymentBankOptions.find((b: any) => b.value === bank);
            bankCode = found?.shorts || '';
        }
        ...
        try {
            ...
            // Format: S${catCode}-${bankCode}-${month number}${String(running).padStart(4, '0')}
            // month number: mon (e.g. '01', '12')
            setForm((prev) => ({
                ...prev,
                receipt_no: `S${catCode}-${bankCode}-${mon}${String(running).padStart(4, '0')}`,
            }));
    }
    ...
    return (
        <form className="w-full max-w-none bg-white dark:bg-neutral-900 p-6 rounded-lg shadow flex flex-col gap-6" onSubmit={handleSubmit}>
            ...
            {/* Payment breakdown */}
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 w-full">
                {companyProfile?.paymentTypes?.map((type: { value: string; label: string }) => {
                    const cashFilled = !!form.payment.cash && parseFloat(form.payment.cash) > 0;
                    const isDisabled = type.value !== 'cash' && cashFilled;
                    return (
                        <div key={type.value}>
...
export default IssueSaleReceiptForm;
