...
const LedgerReport: React.FC<LedgerReportProps> = ({ ledgers, month, year }) => {
  return (
    <>
      {(ledgers ?? []).map((acc, idx) => (
        <div
          ...
          style={{ pageBreakAfter: idx !== ledgers.length - 1 ? 'always' : 'auto', breakAfter: idx !== ledgers.length - 1 ? 'page' : 'auto' }}
        >
          <LedgerTable acc={acc} month={month} year={year} />
        </div>
      ))}
    </>
  );
