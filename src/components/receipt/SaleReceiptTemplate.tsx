...
const SaleReceiptTemplate: React.FC<SaleReceiptTemplateProps> = ({ data, className }) => {
  ...
  return (
    <div className={`w-full max-w-2xl mx-auto bg-white dark:bg-neutral-900 rounded-xl shadow-lg border border-gray-200 dark:border-neutral-700 my-4 ${className || ''}`}>
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center border-b border-gray-200 dark:border-neutral-700 p-4 md:p-6 gap-4">
    ...
    <div className="grid grid-cols-1 md:grid-cols-12 gap-4 p-4 md:p-6">
      <div className="md:col-span-7 bg-gray-50 dark:bg-neutral-800 rounded-lg p-4 border border-gray-100 dark:border-neutral-800">
      ...
      <div className="md:col-span-5 bg-gray-50 dark:bg-neutral-800 rounded-lg p-4 border border-gray-100 dark:border-neutral-800">
    ...
    <div className="overflow-x-auto px-2 md:px-6">
      <table className="min-w-full text-xs border border-gray-200 dark:border-neutral-700 rounded-lg overflow-hidden mb-4">
        ...
        <tbody>
          {(data.products || []).map((p: any, i: number) => (
            <tr key={i}>
              ...
              <td className="p-2 border border-gray-200 dark:border-neutral-700 text-right">{p.weight ?? ''}</td>
              <td className="p-2 border border-gray-200 dark:border-neutral-700 text-right">{p.quantity ?? ''}</td>
    ...
    <div className="grid grid-cols-1 md:grid-cols-12 gap-4 px-4 md:px-6 pb-4">
      <div className="md:col-start-6 md:col-span-7 flex flex-col items-end">
        <div className="bg-gray-50 dark:bg-neutral-800 rounded-lg p-4 border border-gray-100 dark:border-neutral-800 w-full md:w-[340px]">
          <table className="w-full text-xs">
            <tbody>
              <tr>
                <td className="label text-gray-500 dark:text-gray-400">ยอดรวม (Sum of items)</td>
                <td className="num text-right text-gray-700 dark:text-gray-200">{Number((data.products || []).reduce((sum: number, p: any) => sum + Number(p.price || 0), 0)).toLocaleString('th-TH', { minimumFractionDigits: 2 })}</td>
              </tr>
              <tr>
                <td className="label text-gray-500 dark:text-gray-400">ภาษีมูลค่าเพิ่ม (VAT %)</td>
                <td className="num text-right text-gray-700 dark:text-gray-200">{data.vat ? `${data.vat}%` : '-'}</td>
              </tr>
              <tr>
                <td className="label text-gray-700 dark:text-gray-200 font-semibold">ยอดชำระรวม (Grand Total)</td>
    ...
    <div className="px-4 md:px-6 pb-6">
      <div className="text-xs text-gray-500 dark:text-gray-400 mb-2">ช่องทางรับชำระ:
