...
import { formatMoney, isPurchaseType, isSaleType } from "../utils/utils";
...
export default function StockMovementReport() {
  ...
  const totalIn = rows.filter(r => isPurchaseType(r.type)).reduce((sum, r) => sum + (typeof r.total === 'number' ? r.total : parseFloat(r.total || 0)), 0);
  const totalOut = rows.filter(r => isSaleType(r.type)).reduce((sum, r) => sum + (typeof r.total === 'number' ? r.total : parseFloat(r.total || 0)), 0);
  ...
  const typeSummaries = Object.entries(typeGroups).map(([type, groupRows]) => {
    const totalIn = groupRows.filter(r => isPurchaseType(r.type)).reduce((sum, r) => sum + (typeof r.total === 'number' ? r.total : parseFloat(r.total || 0)), 0);
    const totalOut = groupRows.filter(r => isSaleType(r.type)).reduce((sum, r) => sum + (typeof r.total === 'number' ? r.total : parseFloat(r.total || 0)), 0);
